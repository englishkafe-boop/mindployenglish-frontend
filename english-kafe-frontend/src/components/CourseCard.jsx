  ...
  return (
    <div className="bg-white rounded-lg overflow-hidden shadow-md hover:shadow-lg transition-shadow flex flex-col sm:flex-row gap-2 sm:gap-3 p-2 sm:p-3">
      ...
      <div className="flex-1 flex flex-col justify-between min-w-0">
        {/* Title as Badge */}
        <div className="inline-block bg-blue-100 text-blue-700 text-xs font-semibold px-2 py-1 rounded-full mb-1 w-fit flex-shrink-0">
          {title}
        </div>
        ...
        <div className="flex gap-2 flex-col sm:flex-row">
          <button 
            ...
            className="flex-1 px-2 sm:px-4 py-1.5 border-2 border-pink-400 text-gray-700 font-semibold text-xs sm:text-sm rounded-lg hover:bg-gray-50 transition-colors cursor-pointer"
          >
            View details
          </button>
          <button 
            onClick={() => navigate(`/enroll/${id}`)}
            className="flex-1 px-2 sm:px-4 py-1.5 bg-pink-300 text-gray-900 font-semibold text-xs sm:text-sm rounded-lg hover:bg-pink-400 transition-colors"
          >
            Enroll Now
          </button>
        </div>
